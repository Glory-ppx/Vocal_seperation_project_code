...
elseif (strcmp(param.InitializationMethod,'DataElements'))
    Dictionary(:,1:param.K-param.preserveDCAtom) = Data(:,1:param.K-param.preserveDCAtom);%?????1?param.K-param.preserveDCAtom???????
elseif (strcmp(param.InitializationMethod,'GivenMatrix'))
    Dictionary(:,1:param.K-param.preserveDCAtom) = param.initialDictionary(:,1:param.K-param.preserveDCAtom);%?initialDictionary?1?param.K-param.preserveDCAtom??????
end
% reduce the components in Dictionary that are spanned by the fixed
% elements
if (param.preserveDCAtom)
    tmpMat = FixedDictionaryElement \ Dictionary;%?minimal norm?FixedDictionaryElement×Dictionary-dictionary?
...
function [betterDictionaryElement,CoefMatrix,NewVectorAdded] = I_findBetterDictionaryElement(Data,Dictionary,j,CoefMatrix,numCoefUsed)%CoefMatrix????????
if (length(who('numCoefUsed'))==0)
    numCoefUsed = 1;
end
relevantDataIndices = find(CoefMatrix(j,:)); % ?????j??????????the data indices that uses the j'th dictionary element.
if (length(relevantDataIndices)<1) %(length(relevantDataIndices)==0)????????j????
    ErrorMat = Data-Dictionary*CoefMatrix;%?????????????data??????
    ErrorNormVec = sum(ErrorMat.^2);%?? ?????
    [d,i] = max(ErrorNormVec);%d?????????i?????
    betterDictionaryElement = Data(:,i);%ErrorMat(:,i); %????i???betterDictionaryElement
    betterDictionaryElement = betterDictionaryElement./sqrt(betterDictionaryElement'*betterDictionaryElement);%???betterDictionaryElement
    betterDictionaryElement = betterDictionaryElement.*sign(betterDictionaryElement(1));%?betterDictionaryElement?????????
    CoefMatrix(j,:) = 0;%???????j????0
    NewVectorAdded = 1;
    return;
end
...
NewVectorAdded = 0;
tmpCoefMatrix = CoefMatrix(:,relevantDataIndices); %???????j???????????tmpCoefMatrix
tmpCoefMatrix(j,:) = 0;% the coeffitients of the element we now improve are not relevant.?tmpCoefMatrix?j??0
errors =(Data(:,relevantDataIndices) - Dictionary*tmpCoefMatrix); %???????j??????????????????% vector of errors that we want to minimize with the new element
...
[betterDictionaryElement,singularValue,betaVector] = svds(errors,1);%betterDictionaryElement?????? singularValue?????? betaVector?????
CoefMatrix(j,relevantDataIndices) = singularValue*betaVector';% *signOfFirstElem ??????j??????????singularValue*betaVector??
