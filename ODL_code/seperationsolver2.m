%% how to use
% set the rootdir to path of your 'data' folder
% in startind and endind, change the time interval you try to implement
% seperation
% run it! The seperation result will be store in 'data' folder you set
% since the size of music is too large,we only have 1 testing audio
...
rootdir = 'D:\learn\CS591_CompressedSensing\project\data\'; % this is all your data(dictionary and testing set) lies in
testmode=0;%this is for testing. Set 0 to turn off testing mode (it's for debugging so no need to use it)
...
sparsity = 20; %here is the sparsity for solving l-0 minimization problem with OMP
compress_rate=1;%the rate for compression when read in the audio
samplerate = 44100;%the sample rate of the audio
num_sample = samplerate*5;
startind = samplerate*10; %seperation start from 10 sec
endind = samplerate*25; % seperation end at 25 sec
if testmode == 1
...
if exist('D','var') == 0
    ...
    colbg = 3*size(bgD1,2);
    D = [vocalD,bgD1,bgD2,bgD3];
    clear vocalD bgD1 bgD2 bgD3
end
